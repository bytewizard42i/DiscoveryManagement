# DiscoveryManagement — Next Steps
...
**All 6 DM smart contracts are compiled and deployed to Midnight Preprod.** This is a huge milestone. Here's what that means in plain English:
...
**What needs to happen**: Replace the counter SDK with DM-specific providers that talk to our 6 deployed contracts. In plain English, this means:
...
|-------------|-----------|
| `discovery-contract/src/contracts/*.compact` | The smart contract source code (Compact language) |
| `discovery-contract/src/managed/*/` | Compiled contract outputs (JS, keys, zkir) |
| `frontend-realdeal/src/` | The real frontend that will connect to blockchain |
| `frontend-realdeal/src/contracts/` | Where DM contract controllers will live |
